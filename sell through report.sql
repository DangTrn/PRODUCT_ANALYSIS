...
WITH
	...
	NSM AS
		(SELECT	SHIPMENT, [Prod Cd], [Bill Qty], OSD,
			...
			END AS SEA_INDEX
		FROM 
			NEW_SHIPMENT LEFT JOIN ALC 
			ON 
				[Prod Cd]=SKU AND 
				[Sz Desc]=SIZE AND 
				NEW_SHIPMENT.SHIPMENT=ALC.SM),
	...
	SHIPIN AS
		...
		FROM NSM
			INNER JOIN LIST_SEASON ON NSM.SEA = LIST_SEASON.SEA
			INNER JOIN LAST_SEASON ON NSM.[Prod Cd]=LAST_SEASON.[Prod Cd] AND NSM.SEA = LAST_SEASON.LAST_SEA
